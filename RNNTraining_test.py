...
if __name__ =='__main__':
    ...
    print(file_name[7])
    df_test = pd.read_excel(file_name[7], header=None, usecols=[1, 2, 3, 4, 5], names=None)
    tmp_data_y = (df_test.values)[1:,4]
    tmp_data_x = (df_test.values)[1:,0:4]
    ...
    data_test.append(np.array(tmp_data_y[:]))
    data_test_x.append(np.array(tmp_data_x[:]))
    # print(tmp_data_y)
    # print(tmp_data_x)
    ...
    # split into train and test sets, 20% test data, 80% training data, for one palpation test, choose a single back and forth process
    train_size = int(len(dataset) * 0.90)
    # test_size = len(dataset) - train_size
    train_y, test_y = dataset[0:train_size,:], dataset[train_size+10:train_size+60,:]
    train_x,test_x = dataset_x[0:train_size,:], dataset_x[train_size+10:train_size+60,:]
    ...
    if all_x.shape[1] == 4:
        ...
        model.add(LSTM(81, input_shape=(1,3))) # 以及输入维度是 x,y coordinates and depth，三维
    ...
    model.fit(trainX, trainY, epochs=train_epoch, batch_size=150, verbose=1) # 需要修改batchsize,尝试为8
    ...
    if len(testY) >= 80:
        ...
        testPredict = scaler.inverse_transform(testPredict)-0.12
    ...
    plt.figure(1)
    base_curve, = plt.plot(scaler.inverse_transform(dataset[0:train_size, :]))
    ...
    if all_x.shape[1] == 4:
        ...
        base_curve, =plt.plot(a,b)
        # base_curve = ax.scatter3D(c,b,a)
        test_predict_curve, = plt.plot(a,d)
